...
/*1.  (Easy)  List every order with the customer's 
full name, store name, and the full name of the staff member who handled it.*/
...
total revenue per month for the year 2017, ordered chronologically.*/
SELECT 
...
/*9.  (Hard)  Using a CTE, calculate each customer's total spend, then return the 
top 10 customers with their spend and rank. Add a second CTE that labels 
...
SELECT TOP 10
    ...
    cl.total_spend,
    cl.spend_label,
    RANK() OVER (ORDER BY cl.total_spend DESC) AS spend_rank
FROM customer_label AS cl
JOIN sales.customers AS c
